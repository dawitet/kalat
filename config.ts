...
export const GAME_CONFIG = {
    ...
    DIFFICULTY_LEVELS: { // (from File 1, if you use difficulty levels)
        EASY: 'easy',
        HARD: 'hard',
    },
};
...
if (__DEV__) { // __DEV__ is a global variable in React Native, true in development
    console.log(`Configuration loaded: APP_SHARE_URL = ${APP_SHARE_URL}, API_BASE_URL = ${API_CONFIG.BASE_URL}`);
}

// --- Basic Validation (Optional, but good for sanity checks) ---
const simpleValidate = (value: string | undefined | null, name: string) => {
    if (!value || value.includes('YOUR_') || value.includes('example.com') || value.endsWith('/api')) { // Added a check for unreplaced API
        console.warn(`Configuration Check: ${name} might be missing or using a placeholder/default value ('${value}'). Please verify.`);
    }
};
