  ...
  switch (action.type) {
    ...
    case 'INITIALIZE_GAME': {
      ...
      if (!difficulty || !state.targetWords[difficulty]?.word) {
        ...
        return {
          ...state,
          errorMessage:
            'Game cannot start: Word not loaded or difficulty not set.',
        };
      }
      return {
        ...state,
    ...
    case 'ADD_LETTER': {
      ...
      if (
        ...
        state.gameProgress[difficulty].isFinished
      ) {
        return state;
      } // Added curly braces
    ...
    case 'SUBMIT_GUESS': {
      ...
      for (let i = 0; i < WORD_LENGTH; i++) {
        if (guessLetters[i] !== '$') {
          ...
          if (letterIndexInTarget !== -1) {
            ...
            if (!newLetterHints[guessLetters[i]]) {
              // Only mark absent if not already correct/present
      ...
      if (isFinished) {
        ...
        const timeTaken = progress.gameStartTime
          ? (gameEndTime - progress.gameStartTime) / 1000
          : null;
        updatedStats = {
        ...
        if (difficulty === 'easy') {
          updatedStats = {
            ...
            totalGuessesEasy:
              state.totalGuessesEasy + (progress.currentRow + 1),
          ...
          updatedStats = {
            ...
            ...(didWin && {
              ...
              ...(timeTaken &&
                (!state.bestTimeHard || timeTaken < state.bestTimeHard) && {
        ...
        if (didWin && state.dailyStatus.date === getDateString(new Date())) {
          if (difficulty === 'easy') {
            updatedStats = {
              ...updatedStats,
              dailyStatus: {...state.dailyStatus, easyCompleted: true},
            };
          } else if (difficulty === 'hard') {
            updatedStats = {
              ...updatedStats,
              dailyStatus: {...state.dailyStatus, hardCompleted: true},
            };
          }
        }
      }
    ...
    case 'REVEAL_MAIN_HINT': {
      ...
      if (
        !difficulty ||
        !state.gameProgress[difficulty] ||
        state.gameProgress[difficulty].mainHintRevealed
      ) {
        return state;
      } // Added curly braces
    ...
    case 'START_NEW_GAME': {
      // This action might be dispatched from GameView or a modal
      const difficultyToStart =
        action.payload?.difficulty || state.preferredDifficulty || 'easy';
      if (!state.targetWords[difficultyToStart]?.word) {
        return {
          ...state,
          errorMessage: `Cannot start new ${difficultyToStart} game: Word not loaded.`,
        };
      }
      return {
        ...state,
